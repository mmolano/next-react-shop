...
export const UserLog = (prop: { isMobile?: boolean }) => {
  const { user } = useUser();
  // TODO: fix not showing when user not 
  return (
    <div>
      {!prop.isMobile && !user && (
        <li>
          ...
          <li className="list-none">
            <Link href={'/user/profile'}>
              <Image className="rounded-xl w-6 h-6 m-auto"
                src={user.picture ? user.picture : "/public/default-user.svg"}
                height="24"
                width="24"
                alt={user.name ? user.name : "User profile"}
              />
            </Link>
          </li>
        )
